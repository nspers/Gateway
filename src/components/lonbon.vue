                    ...
                    <li class="clearfix">
                        ...
                        <div class="editDiv">
                            ...
                            <FormItem prop="device_type">
                                <Select v-model="useData.device_type" size="large">
...
export default {
        ...
        return{
            ...
            columns1: [
                ...
                },
                {
                    ...
                    render(h,parmes) {
                        let flag;
                        if(parmes.row.device_type == 0){
                            flag = '主机'
                        }else if(parmes.row.device_type == 1){
                            flag = '分机'
                        }else if(parmes.row.device_type == 3){
                            flag = '地址盒'
                        }
                        return h('div',flag)
                    },
    ...
    methods:{
        updataList(){
            var self = this
            this.loading = true
            this.$api.get('sysDevice/selectlonbonList/').then(function(res){
                self.data1 = res.data
                self.loading = false
            })
        },
        ...
        remove(){
            ...
            this.$api.delete('sysDevice/delectlonbon/' + this.data1[index].id).then(function(res){
                self.updataList()
        ...
        editRole(index){
            this.edit = true
            var self = this
            this.$api.get('sysDevice/selectlonbonList/' + this.data1[index].id).then(function(res){
                console.log(res);
                self.showData = true
        ...
        submitUser(){
            ...
            this.$refs.formValidate.validate((valid) => {
                if (valid) {
                    if(this.edit){
                        this.$api.put('sysDevice/updatelonbon',this.useData).then(function(res){
                            console.log(res);
                            self.edit = false
                            self.showData = false
                            self.updataList()
                        ...
                        this.$api.post('sysDevice/savelonbon',this.useData).then(function(res){
                            self.showData = false
                            self.updataList()
                        })                
                    }
