            ...
            <Form ref="formValidate" :model="useData" :rules="this.$ruleInline">
                <ul class="showUser">
                    ...
                    <li class="clearfix">
                        <strong>设备类型：</strong>
                        <div class="editDiv">
                            <!-- <Input size="large" clearable v-model="useData.device_type"  /> -->
                            <FormItem prop="deviceType">
                                <Select v-model="useData.deviceType" size="large">
                    ...
                    <li class="clearfix">
                        ...
                        <div class="editDiv">
                            <FormItem prop="deviceName">
                                <Input size="large" clearable v-model="useData.deviceName"  />
                            </FormItem>
                        </div>
                    </li>
                    <li class="clearfix">
                        ...
                        <div class="editDiv">
                            <FormItem prop="deviceId">
                                <Input size="large" clearable v-model="useData.deviceId"  />
                            </FormItem>
                        </div>
                    </li>
                    <li class="clearfix">
...
export default {
        ...
        return{
            ...
            columns1: [
                ...
                },
                {
                    ...
                    render(h,parmes) {
                        let flag;
                        if(parmes.row.deviceType == '202'){
                            flag = '四项监测'
                        }else if(parmes.row.deviceType == '305'){
                            flag = '八项监测'
                        }
                        return h('div',flag)
                    }
    ...
    methods:{
        updataList(){
            var self = this
            this.loading = true
            this.$api.get('sysDeviceRingmeasurement/selectRingmeasurementList/').then(function(res){
                self.data1 = res.data
                self.loading = false
            })
        },
        ...
        remove(){
            ...
            this.$api.delete('sysDeviceRingmeasurement/delectRingmeasurement/' + this.data1[index].id).then(function(res){
                self.updataList()
        ...
        editRole(index){
            this.edit = true
            var self = this
            this.$api.get('sysDeviceRingmeasurement/selectRingmeasurementList/' + this.data1[index].id).then(function(res){
                console.log(res);
                self.showData = true
        ...
        submitUser(){
            ...
            this.$refs.formValidate.validate((valid) => {
                if (valid) {
                    if(this.edit){
                        this.$api.put('sysDeviceRingmeasurement/updateRingmeasurement',this.useData).then(function(res){
                            console.log(res);
                            self.edit = false
                            self.showData = false
                            self.updataList()
                        ...
                        this.$api.post('sysDeviceRingmeasurement/saveRingmeasurement',this.useData).then(function(res){
                            self.showData = false
                            self.updataList()
                        })                
                    }
