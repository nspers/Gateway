                    ...
                    <li class="clearfix">
                        ...
                        <div class="editDiv">
                            <FormItem prop="deviceType">
                                <Input size="large" clearable v-model="useData.deviceType" />
                            </FormItem>
                        </div>
                    </li>
                    <li class="clearfix">
...
export default {
    ...
    methods:{
        updataList(){
            var self = this
            this.loading = true
            this.$api.get('sysDeviceChint/selectChintList/').then(function(res){
                self.data1 = res.data
                self.loading = false
            })
        },
        ...
        remove(){
            ...
            this.$api.delete('sysDeviceChint/delectChint/' + this.data1[index].id).then(function(res){
                self.updataList()
        ...
        editRole(index){
            this.edit = true
            var self = this
            this.$api.get('sysDeviceChint/selectChintList/' + this.data1[index].id).then(function(res){
                console.log(res);
                self.showData = true
        ...
        submitUser(){
            ...
            this.$refs.formValidate.validate((valid) => {
                if (valid) {
                    if(this.edit){
                        this.$api.put('sysDeviceChint/updateChint',this.useData).then(function(res){
                            console.log(res);
                            self.edit = false
                            self.showData = false
                            self.updataList()
                        ...
                        this.$api.post('sysDeviceChint/saveChint',this.useData).then(function(res){
                            self.showData = false
                            self.updataList()
                        })                
                    }
