  ...
  methods: {
    ...
    register() {
      ...
      if (
        ...
        var self = this;
       this.$api.post('anno/login',{account:this.usename,password:this.password},{withCredentials:true}).then(function(res){
            if(res.code == 0){
                sessionStorage.setItem('tokenGATEWAY',res.seesionId)
                sessionStorage.setItem('username',self.usename)
...
.login_warp {
  width: 100vw;
  height: 100vh;
  background: #000 url("../assets/img/login/login_bg.jpg") no-repeat
    center/cover;
  color: #fff;
  text-shadow: none;
  // position: relative;
