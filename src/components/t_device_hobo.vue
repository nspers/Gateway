...
export default {
    ...
    methods:{
        updataList(){
            var self = this
            this.loading = true
            this.$api.get('sysDeviceHobo/selectHoboList/').then(function(res){
                self.data1 = res.data
                self.loading = false
            })
        },
        ...
        remove(){
            ...
            this.$api.delete('sysDeviceHobo/delectHobo/' + this.data1[index].id).then(function(res){
                self.updataList()
        ...
        editRole(index){
            this.edit = true
            var self = this
            this.$api.get('sysDeviceHobo/selectHoboList/' + this.data1[index].id).then(function(res){
                console.log(res);
                self.showData = true
        ...
        submitUser(){
            ...
            this.$refs.formValidate.validate((valid) => {
                if (valid) {
                    if(this.edit){
                        this.$api.put('sysDeviceHobo/updateHobo',this.useData).then(function(res){
                            console.log(res);
                            self.edit = false
                            self.showData = false
                            self.updataList()
                        ...
                        this.$api.post('/sysDeviceHobo/saveHobo',this.useData).then(function(res){
                            self.showData = false
                            self.updataList()
                        })                
                    }
